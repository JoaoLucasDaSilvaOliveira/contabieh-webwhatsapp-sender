...
export class WppService {
    ...
    static sendMessagesAndFiles = async (contatos: ContatosEmCSV[], arquivos: File[], interval: number, manualMessage?: string) => {
        ...
        for (const contato of contatos){
            ...
            for (const arquivo of arrayTempArquivos){
                ...
                try{
                    await this.sendFile(contato.nome, arquivo.name, manualMessage ? manualMessage : contato.mensagem);
                    wppStates.cleanActualFile();
    ...
    static sendMessages = async (contatos: ContatosEmCSV[] | {nome: string, telefone: string, mensagem: string}[], interval : number, manualMessage?: string) => {
        ...
        for (const contato of contatos){
            ...
            await this.sleep(3000);
            try{
                await this.sendText(contato.telefone, manualMessage ? manualMessage : contato.mensagem)
                wppStates.cleanActualAction();
    ...
    static sendText = async (args1: any, args2: any) =>{
        return new Promise<string>((res, rej) =>{
            setTimeout(()=>{
                // rej('Ocorreu um erro, verifique os logs')
                res('')
            }, 1) //demora 3 seg pra enviar a mensagem
        })
    }

    static sendFile = async (to: string, fileName: string, message: string) => {
        return new Promise<string>((res, rej) =>{
            setTimeout(()=>{
                // rej('Ocorreu um erro, verifique os logs')
                res('')
